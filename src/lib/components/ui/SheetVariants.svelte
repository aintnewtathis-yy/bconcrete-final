<script>
    let { colors, currentColor = $bindable() } = $props();
...
{#snippet colorCard(content)}
    ...
    >
        <div
            ...
            class="rounded w-full min-h-40 border-gray-200 border aspect-[260/200]"
        ></div>
        <p
            class="p-2 rounded border transition duration-300"
            class:bg-transparent={content.color != currentColor.color}
            class:border-[#CCCCCC]={content.color != currentColor.color}
            class:bg-zinc-200={content.color == currentColor.color}
        >
            {content.title}
        </p>
    </button>
...
>
    <form method="POST" action="#" class="flex flex-col gap-10 w-full">
        ...
        <div
            class="grid grid-cols-2 gap-4 overflow-y-auto no-scrollbar overflow-x-hidden"
        >
        ...
        <div
            class="p-4 flex flex-col gap-4 bg-[url('/form-bg.png')] rounded bg-cover bg-center"
        >
            <p class="text-2xl max-sm:text-xl">Не нашли подходящий цвет?</p>
            <p class="texl-lg w-3/5 max-sm:text-base max-sm:w-4/5">
                Напишите нам, и мы подберём для вас нужную конфигурацию
            </p>
            <a href="#" class="bg-black p-2 rounded text-white w-fit">
                Написать в Whatsapp
            </a>
        </div>
        <button
